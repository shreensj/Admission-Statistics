...
function buildSummaryCards() {
  ...
  const generatedAt = state.workbook.generatedAt || baseData.generatedAt || "-";

  const cards = [
    { label: "Total sheets", value: totalSheets },
    { label: "Total records", value: totalRows },
    { label: "Tracked columns", value: totalColumns },
    { label: "Snapshot time", value: generatedAt },
  ];
    ...
    .map((sheet, index) => {
      ...
      return `
        <button class="sheet-tab ${isActive}" data-sheet-index="${index}">
          ...
          <span>${sheet.rows.length} rows | ${sheet.headers.length} columns</span>
        </button>
      `;
    })
    .join("");
...
function setStatus(message, tone = "ok") {
  ...
  statusMessageValue.className = tone === "warn" ? "badge warn" : "badge";
